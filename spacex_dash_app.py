...
# Get the list of unique test site names
...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        fig = px.pie(pie_data, values='Count', names='class', title='Success vs. Failed for All Launch Sites')
    else:
        # Create the pie chart for the selected test site
        site_df = spacex_df[spacex_df['Launch Site'] == entered_site]
        pie_data = site_df.groupby('class').size().reset_index(name='Count')
        fig = px.pie(pie_data, values='Count', names='class', title=f'Success vs. Failed for {entered_site}')
    return fig
